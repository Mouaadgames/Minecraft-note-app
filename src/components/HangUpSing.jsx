...
function HangUpSing({ text, writeable = false, setInputValue, fieldValue, className, windowHeighFactor }) {
  const windowSize = useWindowSize()

  return (
    <div className="select-none flex justify-center -top-4 w-full ">
      <div className={"inline min-h-[150px] relative" + " " + className} style={{ height: windowSize.height / windowHeighFactor || 3.5 }}>
        <img src={hangUpSingImg} alt="" className="h-full" />
        {
          writeable ? (
            ...
            <h1 className={`text-black text-center absolute bottom-[25%] translate-y-1/2 p-2 ${windowSize.height > 800 ? "text-4xl leading-8" : "text-3xl leading-7"} w-full`}>{text}</h1>
          )
        }
      </div>
    </div>
