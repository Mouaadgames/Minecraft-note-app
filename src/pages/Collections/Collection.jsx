    ...
    <div className="flex items-center relative max-w-2xl select-none">
      <Link to={"/collection/" + collectionData.id} className="flex items-center relative max-w-2xl">
        ...
        <div onClick={onClick} className="max-w-[240px] pl-4 cursor-pointer">
          ...
          <span>
            Number Of Books : {collectionData.numOfBooks}
          </span>
        </div>
      </Link>
      <span className="flex-1"></span>
      <div onClick={() => openEditCollection(collectionData)} className="relative self-start top-0 right-0 border-white border-solid border-2 pt-1 pl-[6px] aspect-square w-8 h-8 rounded-full cursor-pointer hover:bg-[#FFF7] ">🖍</div>
    </div>
  )
}
